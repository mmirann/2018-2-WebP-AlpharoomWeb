...
<br>
<div style="border: 1px solid #eeeeee; width:900px; position:relative; margin-left:250px;height: auto; padding:10px;"><%=content.replace("\r\n", "<br>") %></div>
<p>
<div style="position:relative; margin-left:700px">
  <%
  if(session.getAttribute("sessionId") != null){
  String my_id=session.getAttribute("sessionId").toString();
  if(my_id.equals("admin")){ %>
  <input type="button" class="my_button" value="답글" onclick="location.href='Q&A_insert.jsp?ref=<%=ref%>&flag=r'"/>
  <%} }%>
<input type="button" class="my_button" value="목록" onclick="location.href='Q&A.jsp'"/>
<input type="button" class="my_button" value="수정" onclick="location.href='Q&A_modify_pw.jsp?id=<%=id%>'"/>
<input type="button" class="my_button" value="삭제" onclick="location.href='Q&A_delete_pw.jsp?id=<%=id%>'"/>
</div>
</div>
<div id="underbar">
